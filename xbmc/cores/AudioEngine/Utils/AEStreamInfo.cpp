...
void CAEStreamParser::Reset()
{
  ...
  // and forget what the last stretch said, so nothing is published about a
  // presentation until the stream in hand has said it again. Neither codec
  // hands on a packet before then: TrueHD cannot regain sync without the major
  // sync that carries the figures, and GetPacket() scans an E-AC-3 packet
  // before it lets it go.
  m_info.m_hasAtmos = false;
...
{
  // Every path to a finished packet comes through here, and only here is the
  // whole of it known to be in the buffer
  if (m_info.m_type == CAEStreamInfo::STREAM_TYPE_EAC3)
    ScanEAC3Objects(m_buffer, m_fsize);
...
/* object_audio_metadata_payload(), as far as the object count. Everything past
   ...
   object_count is the presentation's element total, the same figure TrueHD
   states as 16ch_channel_count: how many elements the program holds, of which
   the bed is the static part. It is reported alongside the objects because the
   difference between the two is the only description of that bed either format
   gives. */
static int Atmos_ParseOamdObjects(AtmosBitReader& br, int& elements)
{
  ...
  const int objects = Atmos_DynamicObjects(br, objectCount);
  if (objects >= 0)
                                    ...
                                    int& elements)
{
  ...
  /* emdf_container_length counts the bytes that follow the 32 bits of syncword
     ...
     From here that end is the reader's own limit rather than the frame's. The
     terminator is what this function accepts a container on, and one found past
     emdf_container_length is not this container's terminator - harletty walks
     its payloads under the same bound (while position() < block_end_bit), and
     without it a chance syncword match could close on unrelated bits after the
     length it declared. */
  ...
  for (unsigned int payload = 0; payload < 32; ++payload)
  {
    ...
    /* The reader stops at containerEnd, so br.pos cannot have passed it - what
       is left to check is that the payload it just declared fits as well. */
    const unsigned int payloadStart = br.pos;
    if (!br.ok || payloadBits > static_cast<uint64_t>(containerEnd - payloadStart))
      return -1;
    ...
    {
      AtmosBitReader payloadReader{data, payloadStart, payloadEnd};
      objects = Atmos_ParseOamdObjects(payloadReader, objectElements);
    }
                                   ...
                                   int& elements)
{
  ...
  for (unsigned int bit = startBit; bit <= endBit - 16; ++bit)
  {
    ...
    const int objects = Atmos_TryEmdfContainerAt(data, bit, endBit, elements);
    if (objects >= 0)
      return objects;
  }
...
/* Dynamic objects declared by an E-AC-3 dependent substream carrying JOC.
   ...
   Returns the object count, or -1 if this frame carries none we can read, and
   reports the presentation's element total alongside it. */
static int Atmos_ParseEAC3Objects(const uint8_t* data, unsigned int frameBytes, int& elements)
{
  ...
  if (auxPresent && footer.ok)
  {
    ...
    {
      const int objects =
          Atmos_ScanWindowForOamd(data, trailerStart - auxBits, trailerStart, elements);
      if (objects >= 0)
        return objects;
    }
  }

  return Atmos_ScanWindowForOamd(data, 0, frameBits, elements);
}

/* The length of the AC-3 or E-AC-3 syncframe at data, read off its header, or
   0 when data does not begin one. The same arithmetic TrySyncAC3() syncs on. */
static unsigned int AC3_SyncframeBytes(const uint8_t* data)
{
  ...
  if (bsid > 0x11)
    return 0;
  if (bsid > 10)
    return ((((data[2] & 0x7) << 8) | data[3]) + 1) << 1;
  ...
  switch (fscod)
  {
    case 0:
      return bitRate * 4;
    case 1:
      return (320 * bitRate / 147 + (frmsizecod & 1 ? 1 : 0)) << 1;
    default:
      return bitRate * 8;
  }
}
...
/* DD+ Atmos object metadata is not confined to the dependent substream:
   ...
   header comment records stream type 0). Gating on the dependent frame would
   leave exactly that stream reporting nothing, so every E-AC-3 syncframe of the
   packet is read.

   It is read once the packet is whole, from GetPacket(), rather than while
   TrySyncAC3() is still syncing on it. A dependent frame can arrive split, its
   header in one AddData() and the rest in the next, and the rest is then
   collected by the m_skipBytes branch, which hands the packet on without
   syncing again - so a scan at sync time never saw that frame at all.

   Bounded twice over: the result latches once found, and the packets looked at
   are capped, so a stream carrying none costs a fixed amount of work rather than
   a cost per packet. The demuxer's Atmos typing only widens that budget - it is
   not relied on, because it comes from a decoder pass that passthrough playback
   may never have run. */
void CAEStreamParser::ScanEAC3Objects(const uint8_t* data, unsigned int size)
{
  if (m_eac3ObjectsLatched || size < 8)
    return;

  ++m_eac3ScanAttempts;

  for (unsigned int offset = 0; offset + 8 <= size;)
  {
    const unsigned int frameBytes = AC3_SyncframeBytes(data + offset);
    if (frameBytes == 0 || frameBytes > size - offset)
      break;

    int elements = -1;
    const int objects = (data[offset + 5] >> 3) > 10
                            ? Atmos_ParseEAC3Objects(data + offset, frameBytes, elements)
                            : -1;
    if (objects >= 0)
    {
      m_info.m_hasAtmos = true;
      m_info.m_atmosObjects = objects;
      m_info.m_atmosChannels = elements > 0 ? static_cast<unsigned int>(elements) : 0;
      m_eac3ObjectsLatched = true;
      CLog::Log(LOGINFO,
                "CAEStreamParser::ScanEAC3Objects - E-AC3 JOC object metadata found, "
                "atmosChannels: {}, atmosObjects: {} (after {} packet{})",
                elements, objects, m_eac3ScanAttempts, m_eac3ScanAttempts == 1 ? "" : "s");
      return;
    }
  ...
  if (m_eac3ScanAttempts >= EAC3ObjectScanBudget())
  {
    m_eac3ObjectsLatched = true;
    CLog::Log(LOGINFO,
              "CAEStreamParser::ScanEAC3Objects - E-AC3 no object metadata in {} packets, "
