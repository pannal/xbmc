...
bool CAESinkALSA::Initialize(AEAudioFormat &format, std::string &device)
{
  ...
  if (amlDeviceType != AML_NONE)
  {
    ...
    // on AML AUGESOUND does not reliably support 44.1kHz-family clock rates,
    // causing DMA underruns/cutouts. Snap to the nearest 48kHz-family rate so
    // that ActiveAE resamples upstream and the hardware only ever sees 48kHz-
    // family clocks. The snapped rate is returned via outconfig/format.m_sampleRate
    // so ActiveAE will create the correct per-stream resampler automatically.
    if (!m_passthrough && device.substr(0, 10) == "surround71")
    {
      ...
      if (inconfig.sampleRate != format.m_sampleRate)
        CLog::Log(LOGINFO,
                  ...
                  "family unsupported on TDM-B path)",
  ...
  if (GetAMLDeviceType(info.m_displayName) != AML_NONE)
  {
    ...
    else if (info.m_deviceType != AE_DEVTYPE_HDMI)
    {
      if (device.substr(0, 10) == "surround71")
      {
        ...
        // The TDM-B/I2S path used for HDMI multi-channel output on AML AUGESOUND
        // does not reliably support 44.1kHz-family clock rates, causing periodic
        // underruns/cutouts. Remove these rates so ActiveAE resamples to the
        // 48kHz family before the audio reaches the hardware.
