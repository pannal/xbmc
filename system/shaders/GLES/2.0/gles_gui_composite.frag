...
void main()
{
  ...
  // GUI over the HDR overlays, in the output encoding: the overlays are already
  // there, so they pass through untransformed. Straight colour out, as above.
  float alpha = gui.a + hdr.a * (1.0 - gui.a);
  vec3 hdrColor = hdr.a > 0.0 ? min(hdr.rgb / hdr.a, vec3(1.0)) : vec3(0.0);
  result = (result * gui.a + hdrColor * hdr.a * (1.0 - gui.a)) / alpha;
